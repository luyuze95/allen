...
public class EncryptUtil {
    ...
    public static boolean checkPasswordHash(String password, String passwordHash) {
        ...
        for (int i = 0; i < 48; i += 3) {
            cs1[i / 3 * 2] = passwordHash.charAt(i);
            cs1[i / 3 * 2 + 1] = passwordHash.charAt(i + 2);
            cs2[i / 3] = passwordHash.charAt(i + 1);
        }
    ...
    public static void main(String[] args) {
        ...
        // 获取加盐后的MD5值
        String ciphertext = EncryptUtil.generatePasswordHash(plaintext);
        System.out.println("加盐后MD5：" + ciphertext);
        System.out.println("是否是同一字符串:" + EncryptUtil.checkPasswordHash(plaintext, ciphertext));
    }
}
